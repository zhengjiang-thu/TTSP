...
Implements the paper's coverage-selection-utilization loop:
  1. Dual-pool trace sampling (fresh + Evidence-Ledger-guided)
  2. Round-relative critical-token entropy gating
  3. Deterministic dual-tier Evidence Ledger update
  4. Reliability-weighted readout over every retained trace

Rounds 1..N-1 run the full cycle; the final round skips the ledger update and goes
...
from .reliability import (
    DEFAULT_CRITICAL_POSITIONS,
    DEFAULT_ENTROPY_TOP_K,
    filter_traces,
)
from .evidence_ledger import EvidenceLedgerUpdater
...
from .config import compute_fresh_count
...
def _log_round_voting(round_idx: int, voting_results: Dict[str, Any], prefix: str = ""):
    ...
    if res and res.get("answer"):
        print(f"  {prefix}Cumulative vote (rounds 1–{round_idx + 1}): "
...
class TTSPPipeline:
    ...
    def __init__(self, model: str, **vllm_kwargs):
        ...
        default_kwargs = {
            ...
            "max_logprobs": DEFAULT_ENTROPY_TOP_K,
        ...
        self.max_logprobs = int(default_kwargs["max_logprobs"])
        ...
        self.evidence_ledger_updater = EvidenceLedgerUpdater(self.llm, self.processor)
    ...
    def _validate_run_config(
        ...
        max_ledger_traces: int,
        max_previous_ledger_chars: int,
        confirmed_ledger_cap: int,
        conflict_ledger_cap: int,
    ) -> None:
        if rounds < 1:
        ...
        if entropy_top_k < 1:
            raise ValueError("entropy_top_k must be at least 1")
        if critical_positions < 1:
            raise ValueError("critical_positions must be at least 1")
        if max_ledger_traces < 1:
            raise ValueError("max_ledger_traces must be at least 1")
        if max_previous_ledger_chars < 0:
            raise ValueError("max_previous_ledger_chars cannot be negative")
        if confirmed_ledger_cap < 0 or conflict_ledger_cap < 0:
            raise ValueError("Evidence Ledger caps cannot be negative")
    ...
    def run(
        ...
        max_ledger_traces: int = 4,
        fresh_exploration_ratio: float = 0.4,
        max_previous_ledger_chars: int = 8192,
        entropy_top_k: int = DEFAULT_ENTROPY_TOP_K,
        critical_positions: int = DEFAULT_CRITICAL_POSITIONS,
        confirmed_ledger_cap: int = 8,
        conflict_ledger_cap: int = 4,
        max_extraction_traces: Optional[int] = None,
        max_previous_knowledge_chars: Optional[int] = None,
    ) -> TTSPOutput:
        """
        ...
        Args:
            ...
            max_ledger_traces: Number of highest-scoring survivors sent to the ledger updater.
            fresh_exploration_ratio: Independent fresh-trace fraction after round 1.
            entropy_top_k: Decoder alternatives used for truncated token entropy.
            critical_positions: Highest-entropy positions averaged per trace.

        Returns:
            TTSPOutput with voting results and per-round details.
        """
        if max_extraction_traces is not None:
            max_ledger_traces = max_extraction_traces
        if max_previous_knowledge_chars is not None:
            max_previous_ledger_chars = max_previous_knowledge_chars
        self._validate_run_config(
            rounds, budget_per_round, batch_size, filtering_ratio,
            fresh_exploration_ratio, gamma, entropy_top_k,
            critical_positions, max_ledger_traces,
            max_previous_ledger_chars, confirmed_ledger_cap,
            conflict_ledger_cap,
        )
        if entropy_top_k > self.max_logprobs:
            raise ValueError(
                f"entropy_top_k={entropy_top_k} exceeds the engine's "
                f"max_logprobs={self.max_logprobs}; pass a larger "
                "max_logprobs when constructing TTSPPipeline"
            )
        ...
        output = TTSPOutput(config={
            ...
            "fresh_exploration_ratio": fresh_exploration_ratio,
            "gamma": gamma,
            "entropy_top_k": entropy_top_k,
            "critical_positions": critical_positions,
            "max_ledger_traces": max_ledger_traces,
            "max_previous_ledger_chars": max_previous_ledger_chars,
            "confirmed_ledger_cap": confirmed_ledger_cap,
            "conflict_ledger_cap": conflict_ledger_cap,
        })
        ...
        ledger_updater = EvidenceLedgerUpdater(
            self.llm,
            self.processor,
            confirmed_cap=confirmed_ledger_cap,
            conflict_cap=conflict_ledger_cap,
        )
        ...
        for round_idx in range(rounds):
            ...
            print(f"TTSP Round {round_idx + 1}/{rounds}"
                  + (" (final)" if is_last_round else " (with ledger update)"))
            print('='*70)
            ...
            # Build ledger-guided and independent prompts.
            messages_with_ledger = build_perception_messages(
                question, options, base_image_path[0], evidence_ledger,
            ...
            fresh_count = compute_fresh_count(
                round_idx,
                budget_per_round,
                fresh_exploration_ratio,
                bool(evidence_ledger),
            )

            traces = self._generate_traces(
                messages=messages_with_ledger,
            ...
            round_result.trace_tokens = sum(t.get('num_tokens', 0) for t in traces)
            round_result.total_tokens = round_result.trace_tokens

            # Tag each trace with round info so downstream can distinguish rounds
            for t in traces:
                t['round_idx'] = round_idx
                t['evidence_ledger_input'] = evidence_ledger
                t['visual_facts_input'] = evidence_ledger  # compatibility key

            output.generation_time += round_result.generation_time
            output.all_traces.extend(traces)

            # ----- Stage 2: Reliability Filtering -----
            filtered, threshold = filter_traces(
                traces,
                filtering_ratio=filtering_ratio,
                critical_positions=critical_positions,
            )
            ...
            # ----- Stage 3: Evidence Ledger update (skip on last round) -----
            if not is_last_round and filtered:
                ext_start = time.time()
                print("  Updating Evidence Ledger...")
                ledger_update = ledger_updater.update(
                    retained_traces=filtered,
                    question=question,
                    options=options,
                    image_path=base_image_path[0],
                    max_traces_for_context=max_ledger_traces,
                    previous_ledger=evidence_ledger,
                    max_previous_ledger_chars=max_previous_ledger_chars,
                )
                output.extraction_time += time.time() - ext_start
                if ledger_update:
                    evidence_ledger = ledger_update.text
                    round_result.evidence_ledger = evidence_ledger
                    round_result.ledger_tokens = ledger_update.num_tokens
                    round_result.total_tokens += ledger_update.num_tokens
                    output.total_ledger_tokens += ledger_update.num_tokens

            # ----- Per-round cumulative voting (to track refinement) -----
            round_result.voting_results = compute_voting_results(
                output.all_filtered_traces,
                gamma=gamma,
                options=options,
            )
        ...
        output.total_trace_tokens = sum(
            t.get('num_tokens', 0) for t in output.all_traces
        )
        output.total_tokens = output.total_trace_tokens + output.total_ledger_tokens
        output.avg_tokens_per_trace = (
            output.total_trace_tokens / output.total_traces_count
    ...
    def run_batch(
        ...
        Args:
            ...
            max_ledger_traces: Highest-scoring survivors sent to the ledger updater.
            fresh_exploration_ratio: Independent fresh-trace fraction after round 1.

        Returns:
            List of TTSPOutput, one per input sample.
        """
        del save_debug_details  # Debug serialization is handled by TTSPOutput.
        if max_extraction_traces is not None:
            max_ledger_traces = max_extraction_traces
        if max_previous_knowledge_chars is not None:
            max_previous_ledger_chars = max_previous_knowledge_chars
        ...
        for round_idx in range(rounds):
            ...
            for ctx in contexts:
                ...
                messages_with_ledger = build_perception_messages(
                    ...
                    ctx.base_image_path[0], ctx.evidence_ledger,
                    fresh_exploration=False,
                )
                messages_fresh = build_perception_messages(
                    ctx.question, ctx.options,
            ...
            for ctx_i, ctx in enumerate(contexts):
                ...
                for t in traces:
                    t['round_idx'] = round_idx
                    t['evidence_ledger_input'] = ctx.evidence_ledger
                    t['visual_facts_input'] = ctx.evidence_ledger
            ...
            # --- Stage 3: batch Evidence Ledger update (skip last round) ---
            if not is_last_round:
                ext_start = time.time()
                print(f"  Batch Evidence Ledger update for {n_samples} samples ...")
                ...
                for ctx in contexts:
                    ...
                    if latest_filtered:
                        extraction_items.append({
                            'retained_traces': latest_filtered,
                            'question': ctx.question,
                            'options': ctx.options,
                            'image_path': ctx.base_image_path[0],
                            'previous_ledger': ctx.evidence_ledger,
                        })
                    else:
                        extraction_items.append(None)

                ledger_updates = ledger_updater.update_batch(
                    extraction_items,
                    max_traces_for_context=max_ledger_traces,
                    max_previous_ledger_chars=max_previous_ledger_chars,
                )
                ...
                for ctx, ledger_update in zip(contexts, ledger_updates):
                    if ledger_update:
                        ctx.evidence_ledger = ledger_update.text
                        ctx.ledger_tokens += ledger_update.num_tokens
                        ctx.per_round_results[-1].evidence_ledger = ledger_update.text
                        ctx.per_round_results[-1].ledger_tokens = ledger_update.num_tokens
                        ctx.per_round_results[-1].total_tokens += ledger_update.num_tokens
        ...
        for ctx in contexts:
            ...
            output.total_ledger_tokens = ctx.ledger_tokens
            ...
            output.total_trace_tokens = sum(
                t.get('num_tokens', 0) for t in ctx.all_traces
            )
            output.total_tokens = (
                output.total_trace_tokens + output.total_ledger_tokens
            )
            output.avg_tokens_per_trace = (
                output.total_trace_tokens / output.total_traces_count
                if output.total_traces_count else 0.0
    ...
    def _generate_traces(
        ...
        fresh_count: int = 0,
        logprobs: int = DEFAULT_ENTROPY_TOP_K,
    ) -> List[Dict[str, Any]]:
        """Generate `budget` parallel traces in batches and return processed trace dicts.

        Args:
            messages: List of message dicts for ledger-guided traces.
            fresh_messages: List of message dicts for independent fresh traces.
                           If None, all traces use `messages`.
            fresh_count: Number of independent fresh traces.
        ...
        for batch_start in range(0, budget, batch_size):
            ...
            for offset in range(batch_end - batch_start):
                ...
                global_idx = round_offset + local_idx
                params = copy.deepcopy(sampling_params)
                params.n = 1
                params.logprobs = logprobs
                params.seed = base_seed + global_idx

                # Use the independent prompt for the fresh pool.
                is_fresh = local_idx < fresh_count
